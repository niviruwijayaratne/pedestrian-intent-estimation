...
class LidarProcessing:
    def __init__(self, resolution=0.1, side_range=(-20., 20.), fwd_range=(-20., 20.),
        ...
        self.x_front_detect = float('nan')
        self.y_front_detect = float('nan')
    ...
    def construct_birds_eye_view(self, data):
        ...
        filter_detect = np.logical_and((y_points>-5), (y_points<5))
        filter_detect = np.logical_and(filter_detect, x_points > 0)
        filter_detect = np.logical_and(filter_detect, pixel_vals > 128)
        indices = np.argwhere(filter_detect).flatten()

        self.x_front_detect = np.mean(x_points[indices])
        self.y_front_detect = np.mean(y_points[indices])
        ...
        center = self.convert_to_image(self.x_front_detect, self.y_front_detect)
        cv2.circle(img, center, 5, (0,255,0), -1, 8, 0)
        if not np.isnan(self.x_front_detect) and not np.isnan(self.y_front_detect):
            cv2.arrowedLine(img, (self.vehicle_x, self.vehicle_y), center, (255,0,0))
    ...
    def processLidar(self):
        ...
        detect = np.sqrt(self.x_front_detect**2 + self.y_front_detect**2)
